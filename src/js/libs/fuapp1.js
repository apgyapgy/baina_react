define(function (){
	var isDebug = false;//true取虚拟数据,false调方法从原生取数据
	if(window.location.origin == 'http://localhost:8020' || window.location.origin == 'http://192.168.42.33'){
	  	isDebug = true;
	}
	...
	if(isDebug){
		...
		fuApp = {
			...
			//APP分享小程序到微信
			shareWxXapp: function (success, failed,shareInfo ) {
				var obj = {rspCode:"0000",rspDesc:"成功"};
				success(obj);
			},
	        //调个人资料
		...
		fuApp = {
				...
				userInfo: function (success, failed) {
					console.log("fuapp cordova:",cordova,cordova.exec);
					cordova.exec(function(data){
						if(Android){
							success(JSON.parse(data));
						}else if(iOS){
				...
				startScanCode:function(success,failed,dataInfo){
					cordova.exec(function(data){
						if(Android){
							success(JSON.parse(data));
						}else if(iOS){
					...
					},failed,'FuApp','startScanCode',[dataInfo]);
				}
		}
		return fuApp;
	}
